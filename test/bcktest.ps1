...
Indicate where the .sav files generated by the Satisfactory dedicated server are located. Depending on the installation mode and manual launch mode versus service mode, the folder may change! In: %LOCALAPPDATA%\FactoryGame\Saved\SaveGames
...
Option that will create a task in the task scheduler so that the script restarts every hour.

.PARAMETER RemoveTask
Option that will remove a task in the task scheduler.
